...
mmh <- read.csv("~/github-web/peak-neo4j/misc/maps-minor-historical-cleaned.csv", header = TRUE) %>% select(-c(Major2, Concentration))
...
Of course, we'll also need some data, I'll be using a data set of "anonymized" records of students major/minors degree info from the College of Idaho, which I had in another project: [data](https://github.com/jpreszler/peak-neo4j/misc/maps-minor-historical-cleaned.csv). I've read it in with `read.csv()` and dropped 2 columns:
```{r }
...
## Improving Colors
Removing the tick marks is easy (set option `showTicks=FALSE`), but coming up with lots of colors is a bit harder. The table has dimensions `r dim(majMinTab)`, so we would like `r sum(dim(majMinTab))` colors, and this isn't even all possible combinations that C of I students have obtained!
